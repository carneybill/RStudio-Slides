...
.left-column[
Open-Source
- Integrated Development Environment for R (i.e. like Eclipse or Visual Studio but for R). Runs on the Desktop (Windows, OSX, Linux) or over the Web as a server to enable shared resources and collaboration
  - Data viewer -- support large datasets, filtering, searching, sorting
  - Code completion & Code diagnostics
...
- Push-button publishing from the RStudio IDE that manages all the content types you produce in R: Shiny, R Markdown, plots, etc.
  - Makes it feasible to share data science artifacts
- Whats New?
